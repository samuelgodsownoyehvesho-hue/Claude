...
// high demand" error. That's not a bug on our end — it's Google's servers
// being temporarily overloaded — so retry a couple of times with backoff
...
async function generateContentWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const isOverloaded = err?.status === 503 || /UNAVAILABLE|high demand/i.test(err?.message || "");
      if (isOverloaded && attempt < retries) {
        console.warn(`[Gemini Retry] Model overloaded (attempt ${attempt + 1}/${retries + 1}), retrying in ${delayMs}ms...`);
  ...
  throw new Error("generateContentWithRetry exhausted retries");
}
...
// ---------------------------------------------------------------------------
// Song recognition via ACRCloud — identifies the uploaded track's real title
// and artist from its audio fingerprint (not just embedded file metadata,
// which can be missing, wrong, or edited). On a match, the frontend locks
// the Song Title / Artist fields so they can't be manually changed. If the
// track isn't in ACRCloud's catalog (common for indie/unreleased music),
// we return matched: false and the frontend leaves those fields as-is.
// ---------------------------------------------------------------------------
app.post("/api/recognize", upload.single("audio"), async (req: any, res: any) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: "No audio file uploaded" });
    }
    ...
    if (!host || !accessKey || !accessSecret) {
      return res.status(200).json({ matched: false, reason: "ACRCloud is not configured on the server." });
    }
    ...
    const formData = new FormData();
    const sampleBlob = new Blob([req.file.buffer], { type: req.file.mimetype });
    formData.append("sample", sampleBlob, req.file.originalname || "audio.mp3");
    formData.append("access_key", accessKey);
    formData.append("data_type", dataType);
    formData.append("signature_version", signatureVersion);
    formData.append("signature", signature);
    formData.append("sample_bytes", req.file.buffer.length.toString());
    formData.append("timestamp", timestamp);

    console.log(`[ACRCloud] Identifying track against ${host}...`);
    const acrResponse = await fetch(`https://${host}/v1/identify`, {
      method: "POST",
      body: formData as any
    });
    const acrData: any = await acrResponse.json();

    const musicMatch = acrData?.metadata?.music?.[0];
    if (acrData?.status?.code === 0 && musicMatch) {
      const title = musicMatch.title || null;
      const artist = (musicMatch.artists || []).map((a: any) => a.name).filter(Boolean).join(", ") || null;
      const album = musicMatch.album?.name || null;
      ...
      return res.json({
        ...
        artist,
        album,
        score: musicMatch.score ?? null
      });
    }

    // status.code 1001 = "no result found" — a normal, expected outcome, not an error.
    console.log(`[ACRCloud] No match found (status: ${acrData?.status?.msg || "unknown"}).`);
    return res.json({ matched: false });
  } catch (error: any) {
    console.error("[ACRCloud] Recognition request failed:", error.message);
    // Recognition failing should never block the user's workflow — just report no match.
    res.json({ matched: false, reason: "Recognition service unavailable." });
  }
});
...
app.post("/api/transcode", upload.single("video"), async (req: any, res: any) => {
  ...
  try {
    ...
    // We cap the longest side at 1920px: Railway's container has limited memory, and encoding
    // full 4K (2160x3840) with libx264 was getting OOM-killed (exit code 137) mid-encode.
    // 1080p is already more than enough quality for social platforms (Instagram/TikTok/YouTube).
    // We also explicitly cap -threads: an auto-detected thread count of 70+ in a constrained
    // container multiplies libx264's memory usage far beyond what the container actually has.
    const scaleFilter = "scale='min(1920,iw)':'min(1920,ih)':force_original_aspect_ratio=decrease,scale=trunc(iw/2)*2:trunc(ih/2)*2";
    const ffmpegCmdWithAudio = `"${FFMPEG_BIN}" -i "${inputPath}" -vf "${scaleFilter}" -c:v libx264 -preset superfast -threads 2 -pix_fmt yuv420p -c:a aac -b:a 128k -y "${outputPath}"`;
    ...
    try {
      ...
      const ffmpegCmdNoAudio = `"${FFMPEG_BIN}" -i "${inputPath}" -vf "${scaleFilter}" -c:v libx264 -preset superfast -threads 2 -pix_fmt yuv420p -an -y "${outputPath}"`;
...
// ---------------------------------------------------------------------------
// MCP / external automation API (authenticated)
// ---------------------------------------------------------------------------
