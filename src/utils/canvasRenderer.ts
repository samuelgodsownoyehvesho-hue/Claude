...
export class BackgroundState {
  ...
  public draw(
    ...
    // Uploaded video background takes over entirely — draw the current frame
    // (seeked to match playback time, looping if shorter than the song) with
    // a subtle dark overlay for lyric legibility, and skip particle effects.
    if (config?.backgroundVideoUrl) {
      if (this.videoUrl !== config.backgroundVideoUrl) {
        this.videoUrl = config.backgroundVideoUrl;
        const v = document.createElement("video");
        v.src = config.backgroundVideoUrl;
        v.muted = true;
        v.loop = true;
        v.playsInline = true;
      ...
      if (v && v.readyState >= 2 && v.duration && isFinite(v.duration)) {
        try {
      ...
      // Dark overlay keeps lyric text readable over busy video footage.
      ctx.fillStyle = "rgba(0,0,0,0.35)";
      ctx.fillRect(0, 0, w, h);
      return;
    } else if (this.videoEl) {
      // Video background was removed — release the element.
      this.videoEl.pause();
      this.videoEl.src = "";
      this.videoEl = null;
      this.videoUrl = "";
...
function drawGlitchShake(
  ...
  const isGlitched = Math.floor(time * 15) % 3 === 0;
