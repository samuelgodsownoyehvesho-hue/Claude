...
interface AudioUploaderProps {
  ...
  onSongRecognized?: (title: string, artist: string) => void;
}
...
export default function AudioUploader({ onUploadSuccess, onSongRecognized }: AudioUploaderProps) {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      // Fire song recognition in parallel — it never blocks or delays the
      // transcription flow, and a failure/no-match here is silently ignored
      // since most uploads (indie/unreleased tracks) simply won't be in
      // ACRCloud's catalog. This is best-effort metadata verification only.
      if (onSongRecognized) {
        const recognizeFormData = new FormData();
        recognizeFormData.append("audio", file);
        fetch(`${API_BASE}/api/recognize`, { method: "POST", body: recognizeFormData })
          .then((r) => (r.ok ? r.json() : null))
          .then((result) => {
            if (result?.matched && result.title) {
              onSongRecognized(result.title, result.artist || "Unknown Artist");
            }
          })
          .catch(() => {
            // Recognition is a nice-to-have, not required — ignore failures.
          });
      }
