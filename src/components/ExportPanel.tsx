...
export default function ExportPanel({
  ...
  const [exportExtension, setExportExtension] = useState("webm");
    ...
    try {
      ...
      const renderLoop = (timestamp: number) => {
        ...
        if (timestamp - lastFrameTimeRef.current >= frameInterval) {
          ...
          bgStateRef.current.draw(ctx, time, renderW, renderH, config.backgroundEffect, palette, config);
          ...
          <div>
            ...
            <div className="bg-slate-950/60 border border-slate-800/80 p-3 rounded-xl flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-xs font-semibold text-slate-200">WebM Video</span>
                <span className="text-[10px] text-slate-400 mt-0.5">Fast, direct export straight from your browser — no server processing</span>
              </div>
              <span className="text-[10px] text-indigo-400 font-bold bg-indigo-950/40 px-2 py-1 rounded border border-indigo-900/40 font-mono">VP8/VP9 WebM</span>
            </div>
          </div>
