    ...
    <div className="flex flex-col h-full bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-800/80 p-5 overflow-y-auto shadow-xl" id="styling-settings-sidebar">
      ...
      <div className="mb-5 bg-slate-950/40 border border-slate-800/60 rounded-xl p-3 animate-fade-in" id="metadata-branding-settings">
        ...
        <div className="space-y-1 mb-3">
          <span className="text-[10px] font-bold text-slate-400 uppercase font-mono flex items-center gap-1">
            Song Title
            {config.songMetadataVerified && (
              <span className="inline-flex items-center gap-0.5 text-emerald-400 normal-case font-semibold">
          ...
          <input
            ...
            readOnly={!!config.songMetadataVerified}
            onChange={(e) => onChangeConfig({ songTitle: e.target.value })}
            className={`w-full bg-slate-950 border text-slate-200 text-xs font-medium rounded-lg px-2.5 py-1.5 outline-none transition-colors ${
              config.songMetadataVerified
                ? "border-emerald-500/40 cursor-not-allowed opacity-90"
                : "border-slate-800 focus:border-indigo-500"
            }`}
          />
          {config.songMetadataVerified && (
            <p className="text-[10px] text-slate-500">
              Matched against a rights-holder database — locked to prevent misattribution.
            </p>
          )}
        </div>
      ...
      <div className="mb-5" id="background-effect-picker">
        ...
        <select
          ...
          disabled={!!config.backgroundVideoUrl}
          className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-xs font-semibold rounded-lg px-3 py-2.5 outline-none focus:border-indigo-500 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        {config.backgroundVideoUrl && (
          <p className="text-[10px] text-slate-500 mt-1.5">
            Disabled while a video background is active — remove it below to use a scene effect instead.
          </p>
        )}
      </div>

      {/* 2b. Video Background Upload */}
      <div className="mb-5" id="background-video-upload">
        ...
        {!config.backgroundVideoUrl ? (
          <label className="flex items-center justify-center gap-2 w-full bg-slate-950 border border-dashed border-slate-700 hover:border-indigo-500 text-slate-300 text-xs font-semibold rounded-lg px-3 py-3 cursor-pointer transition-colors">
            <Video className="w-4 h-4 text-slate-500" />
            Upload a video to use as the background
            <input
              type="file"
              accept="video/*"
              className="hidden"
              onChange={(e) => {
                ...
                onChangeConfig({ backgroundVideoUrl: url, backgroundVideoName: file.name });
              }}
            />
          </label>
        ) : (
          <div className="flex items-center justify-between gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-2.5">
            <span className="text-xs text-slate-300 truncate">{config.backgroundVideoName || "Video background"}</span>
            <button
              type="button"
              onClick={() => onChangeConfig({ backgroundVideoUrl: undefined, backgroundVideoName: undefined })}
              className="text-[11px] font-bold text-rose-400 hover:text-rose-300 shrink-0"
            >
              Remove
            </button>
          </div>
        ...
        <p className="text-[10px] text-slate-500 mt-1.5">
          Plays muted and loops behind your lyrics — a dark overlay is applied automatically to keep text readable.
        </p>
      </div>
