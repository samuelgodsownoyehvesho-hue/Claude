...
export default function App() {
  ...
  const handleUploadSuccess = (url: string, file: File, newLyrics: LyricLine[], isFallback?: boolean, reason?: string) => {
    ...
    // Reset verification for the new track — the recognition request (fired
    // in parallel by AudioUploader) will re-set this if it finds a match.
    setConfig((prev) => ({ ...prev, songMetadataVerified: false }));
  ...
  // Called when ACRCloud identifies the uploaded track against its real
  // catalog metadata. Locks Song Title / Artist from manual editing so the
  // credited title/artist can't be misattributed once independently verified.
  const handleSongRecognized = (title: string, artist: string) => {
    setConfig((prev) => ({
      ...prev,
      songTitle: title,
      songArtist: artist,
      songMetadataVerified: true,
    }));
  };
            ...
            <AnimatePresence mode="wait">
              {!lyrics ? (
                ...
                <motion.div
                  ...
                  className="h-full"
                >
                  <AudioUploader onUploadSuccess={handleUploadSuccess} onSongRecognized={handleSongRecognized} />
                </motion.div>
              ) : (
                // 2. Tabbed States
                <>
